...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\uwucoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\uwucoin
    // Mac: ~/Library/Application Support/uwucoin
    // Unix: ~/.uwucoin
#ifdef WIN32
    // Windows
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No uwucoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override uwucoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "uwucoin.pid"));
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                if (!fMatch)
                {
                    ...
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("uwucoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
